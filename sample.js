...
const keywords = ["all", "tagged", "checked", "unchecked"];
  ...
  list: function (term = "all") {
      ...
      if (keywords.includes(arguments[0])) {
        switch (arguments[0]) {
          case "all":
            ...
            break;
          case "tagged":
            items = Object.entries(data).filter((item) =>
              item[1].tags.includes(arguments[1])
            );
            break;
          case "checked":
            items = Object.entries(data).filter((item) => item[1].status);
            break;
          case "unchecked":
            items = Object.entries(data).filter((item) => !item[1].status);
            break;
    ...
    return items.map((task) =>
      process.stdout.write(
        `${toBase36(task[0])} ${task[1].status ? "✔" : "✘"} ${task[1].title} ${
          task[1].tags.length ? "tagged: " + task[1].tags.join(", ") : ""
        }\n`
      )
    );
  },
  ...
  tag: function (id, string) {
    ...
    if (data[fromBase36(id)]) {
      if (data[fromBase36(id)].tags.includes(string)) {
        data[fromBase36(id)].tags.splice(
          data[fromBase36(id)].tags.indexOf(string),
          1
        );
      } else {
